...
class DFPQueryHandler(BaseQueryHandler):
    def _setup_agent(self):
        prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a specialized assistant for DFP queries and Beckn retail processes. "
                    "DFP stands for Demand Flexibility Program - in which users can save on their electricity bills by reducing their peak power demand at a fixed price.\n\n"
                    "You can guide users through a multi-step process for DFP item purchasing: select, confirm and then update.\n\n"
                    "BECKN RETAIL FLOW:\n"
                    "1. SEARCH: To search for DFP plans, use the 'beckn_search' tool with the domain = 'deg:schemes' and the descriptor = 'Program'. And reply with the available items in the response, including provider id, product id, and fulfillment id.\n"
                    ...
                    "2. USER CONFIRMATION: After a successful search, ask if the user wishes to proceed with the selected item."
                    ...
                    "- For DFP, after successful confirmation, the message should congratulate the user for entering into the DFP program. "
                    ...
                    "- If the question is unrelated to DFP or this retail flow, politely state your specialization.",
                ),
        ...
        if not self.llm:
            raise ValueError("LLM not initialized for DFPQueryHandler")
        if not self.tools:
            print(
                "Warning: DFPQueryHandler initialized with no tools, but it's expected to use them."
            )
        ...
        try:
            ...
            print(
                f"Could not create OpenAI tools agent for DFPHandler (LLM: {self.llm_config.model_name}): {e}. Ensure the model supports tool calling."
            )
            ...
            simple_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "You are a specialized assistant for DFP queries.",
                    ),
                    MessagesPlaceholder(variable_name="chat_history", optional=True),
                    ("human", "{input}"),
                ]
            ...
            print(
                "Warning: Agent creation for DFPQueryHandler failed. Tool usage might be affected."
            )
            return
    ...
    async def handle_query(self, query: str, chat_history: InMemoryChatHistory) -> str:
        if not self.agent_executor:
            return "I am currently unable to process your DFP-related request due to an internal setup issue."
        ...
        try:
            ...
            # traceback.print_exc()
            return f"An error occurred while processing your DFP query in {self.__class__.__name__}. Please try again."
