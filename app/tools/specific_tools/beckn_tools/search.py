...
class BecknSearchTool(BaseTool):
    name: str = "beckn_search"
    description: str = (
    ...
    args_schema: Type[BaseModel] = BecknSearchSearchInput
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        descriptor = constants.CONTEXT_DOMAINS_TO_DESCRIPTOR_MAPPING[domain][0]

        payload = {
            "context": {
                "domain": domain,
                "action": constants.CONTEXT_ACTIONS[0],  # search
                "location": {
            ...
            "message": {"intent": {"item": {"descriptor": {"name": descriptor}}}},
        }
        ...
        try:
            response = requests.post(
                f"{constants.BASE_URL}{constants.CONTEXT_ACTIONS[0]}",
                headers=headers,
